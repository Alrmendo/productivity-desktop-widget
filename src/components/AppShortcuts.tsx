...
export default function AppShortcuts({
  ...
  const [iconType, setIconType] = useState<any>('browser');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Fast sanitize url protocol if needed
    let formattedUrl = url.trim();
    if (!/^https?:\/\//i.test(formattedUrl) && !/^[a-zA-Z]:\\/i.test(formattedUrl)) {
  ...
  const handleOpenShortcut = (url: string) => {
    // In actual Electron context, you would use: ipcRenderer.send('open-external', url)
    // Here we simulate via window.open cleanly
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-2.5 bg-gray-50 p-2.5 rounded-xl border border-gray-200">
          ...
          <div>
            ...
            <input
              ...
              placeholder="github.com hoặc đường dẫn hệ thống..."
              className="w-full px-2.5 py-1 text-xs border border-gray-200 bg-white rounded-md outline-none focus:border-gray-400"
              required
            />
          </div>
